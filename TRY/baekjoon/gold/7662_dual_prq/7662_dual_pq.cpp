...
int main(){
	...
	for(int t = 0 ; t < T ; t++){
		storage.clear();
		std::priority_queue<long long> max_pq ;
		std::priority_queue<long long> min_pq ; //push & top (item * -1) 
		int N ; std::cin >> N;
		for(int i = 0 ; i < N + 2; i++){
			char opt ; long long  opr ; 
			if(i == N){
				opt = 'D' ; opr = 1 ; 
			} 
			else if(i == N+1){
			...
			if(opt == 'I'){
				...
				if(opr == -1){
					while(true){
						...
						long long min_val = min_pq.top() * -1 ;
						min_pq.pop();
